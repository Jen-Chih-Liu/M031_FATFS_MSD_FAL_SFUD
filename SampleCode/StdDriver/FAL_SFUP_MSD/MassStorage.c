...
void MSC_Init(void)
{
    ...
    g_sCSW.dCSWSignature = CSW_SIGNATURE;
     ...
     g_TotalSectors = fal_partition_size("bank1") / UDC_SECTOR_SIZE; 
}
        ...
        else
        {
           ...
           // MSC_ReadMedia(g_u32LbaAddress, u32Len, (uint8_t *)STORAGE_DATA_BUF);
            //SpiRead(g_u32LbaAddress, u32Len, (uint32_t) STORAGE_DATA_BUF);
						FAL_FLASH_Read("bank1",g_u32LbaAddress,u32Len,(uint32_t) STORAGE_DATA_BUF);
        ...
        if (g_u32Length > EP3_MAX_PKT_SIZE)
        {
            ...
            if (g_u32Address >= (STORAGE_DATA_BUF + STORAGE_BUFFER_SIZE))
            {
              ...
              // SpiWrite(g_u32DataFlashStartAddr, STORAGE_BUFFER_SIZE, (uint32_t)STORAGE_DATA_BUF);
							 FAL_FLASH_Write("bank1",g_u32DataFlashStartAddr,STORAGE_BUFFER_SIZE,(uint32_t)STORAGE_DATA_BUF);
                ...
                if (len)
                {
                    //DataFlashWrite(g_u32DataFlashStartAddr, len, (uint32_t)STORAGE_DATA_BUF);
						//SpiWrite(g_u32DataFlashStartAddr, len, (uint32_t)STORAGE_DATA_BUF);
									 FAL_FLASH_Write("bank1",g_u32DataFlashStartAddr,len,(uint32_t)STORAGE_DATA_BUF);
                }
            }
            ...
            switch (g_sCBW.u8OPCode)
            {
                ...
                case UFI_READ_10:
                {
                   ...
                   // MSC_ReadMedia(g_u32Address * UDC_SECTOR_SIZE, i, (uint8_t *)STORAGE_DATA_BUF);
										//SpiRead(g_u32Address * UDC_SECTOR_SIZE, i, (uint32_t) STORAGE_DATA_BUF);
										FAL_FLASH_Read("bank1",g_u32Address * UDC_SECTOR_SIZE,i,(uint32_t) STORAGE_DATA_BUF);
